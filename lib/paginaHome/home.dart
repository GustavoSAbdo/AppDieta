...
class _HomePageState extends State<HomePage> {
  ...
  void addFoodToRefeicao(int refeicaoIndex, FoodItem foodItem) {
  setState(() {
  ...
  String today = DateTime.now().toIso8601String().substring(0, 10); // Apenas a data, sem hora

  if (lastResetDate != today) {
    // Se a data do último reset não for hoje, resete as refeições
    setState(() {
      refeicoes = List<Refeicao>.generate(numRef, (_) => Refeicao()); // numRef deve ser atualizado a partir do Firebase
    });
